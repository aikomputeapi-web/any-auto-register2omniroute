...
def dump_visible_inputs():
    ...
    (() => {
        try {
            return JSON.stringify(Array.from(document.querySelectorAll('input, select, textarea, button, a, [role="button"]'))
    ...
    (() => {
        try {
            ...
            for (const clickable of clickables) {
                ...
                // Walk up to 5 parents to find price tag info associated with this button/link
                for (let i = 0; i < 5; i++) {
                    if (!current) break;
                ...
                // Match prices like $19.99, $35, $150.00
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Automate HP Laptop Subscription registration (highest tier).")
    ...
    print("=" * 60)
    print("NEW HP LAPTOP SUBSCRIPTION REGISTRATION (HIGHEST TIER)")
    print("=" * 60)
    ...
    filename = os.path.join(results_dir, f"hp_subscription_details_line_{args.line}.txt")
    ...
    for step_num in range(1, 30):
        ...
        # Check if the program has ended / been discontinued
        if "subscription-faq.html" in url or "programs will end" in body.lower() or "discontinued" in body.lower():
            print("\n[OUTCOME] [DISCONTINUED] The HP Laptop Subscription program has ended/been discontinued.")
            save_details(filename, user_info, email, phone, url, title, "Discontinued / Program Ended", selected_tier)
            break
