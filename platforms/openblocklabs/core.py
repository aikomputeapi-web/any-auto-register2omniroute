"""
OpenBlockLabs Automatic registration (WorkOS AuthKit)
...
process:
  ...
  9. GET /api/create-personal-org → Finish
...
class OpenBlockLabsRegister:
    ...
    def step1_initiate_signup(self) -> bool:
        ...
        for attempt in range(5):
            ...
            if r.status_code == 200:
                break
            self.log(f"  CFintercept (status={r.status_code}), Try again {attempt + 1}/5...")
            time.sleep(2)
        final_url = str(r.url)
    ...
    def step2_get_signup_page(self) -> bool:
        """Already in step1 Completed, return directly True"""
        return bool(self.authorization_session_id)
    ...
    def step5_submit_password(
        ...
        """POST /sign-up/password → RSC body Include pendingAuthenticationToken"""
