...
@register
class BasetenPlatform(BasePlatform):
    name = "baseten"
    ...
    def register(self, email: str, password: Optional[str] = None) -> Account:
        from platforms.baseten.core import BasetenRegister

        log_fn = getattr(self, "_log_fn", print)
        proxy = self.config.proxy
        requested_headless = (self.config.executor_type or "headless") != "headed"

        reg = BasetenRegister(proxy=proxy, headless=requested_headless)
        reg.log = lambda msg: log_fn(f"[Baseten] {msg}")
        ...
        if self.mailbox:
            ...
            def otp_cb():
                ...
                code = mailbox.wait_for_code(
                    ...
                    timeout=otp_timeout,
                    before_ids=_before,
                    # Baseten sends a 6-digit OTP code for email verification
        ...
        if not ok:
            raise RuntimeError(f"Baseten registration failed: {info.get('error')}")
