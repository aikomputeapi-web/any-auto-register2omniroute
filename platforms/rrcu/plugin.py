...
@register
class RRCUPlatform(BasePlatform):
    name = "rrcu"
    display_name = "Red River Credit Union"
    version = "1.0.0"
    ...
    def register(self, email: str, password: Optional[str] = None) -> Account:
        ...
        log(f"Starting RRCU registration for line {line_str} (start line: {start_line}, task index: {task_index}) with phone {phone}...")

        try:
            from pro_account_register import register_rrcu
            import importlib
            importlib.reload(register_rrcu)

            # Run the script's main function
            register_rrcu.main()
        except Exception as e:
            log(f"Error executing RRCU script: {e}")
            raise e
        finally:
            sys.argv = old_argv
