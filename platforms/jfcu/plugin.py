...
@register
class JFCUPlatform(BasePlatform):
    name = "jfcu"
    display_name = "Justice Federal Credit Union"
    version = "1.0.0"
    ...
    def register(self, email: str, password: Optional[str] = None) -> Account:
        ...
        log(f"Starting JFCU registration for line {line_str} (start line: {start_line}, task index: {task_index}) with phone {phone}...")
        
        try:
            from pro_account_register import register_jfcu
            import importlib
            importlib.reload(register_jfcu)
            
            # Run the script's main function
            register_jfcu.main()
        except Exception as e:
            log(f"Error executing JFCU script: {e}")
            raise e
        finally:
            sys.argv = old_argv
