...
@register
class AMEXPlatform(BasePlatform):
    name = "amex"
    ...
    def register(self, email: str, password: Optional[str] = None) -> Account:
        ...
        log(f"Starting AMEX registration for line {line_str} (start line: {start_line}, task index: {task_index}) with phone {phone}...")
        
        try:
            from pro_account_register import register_amex
            import importlib
            importlib.reload(register_amex)
            
            register_amex.main()
        except Exception as e:
            log(f"Error executing AMEX script: {e}")
            raise e
        finally:
            sys.argv = old_argv
